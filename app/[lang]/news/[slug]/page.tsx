...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> {
    ...
    try {
        ...
        return {
            ...
            openGraph: {
                title: `${data.metadata.title} - Apollo`,
                description: data.metadata.description,
                url: `${SITE_URL}/${lang}/news/${slug}`,
                siteName: "Apollo",
                images: [
                    {
                        url: data.metadata.og_image || data.image,
                        width: 1200,
                        height: 630,
                    },
                ],
...
export default async function NewsDetailPage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
    ...
    try {
        ...
        return (
            <main className="w-full bg-white px-4 py-20 text-center">
                <h1 className="text-2xl font-bold text-gray-800">Article Not Found</h1>
                <Link href={`/${lang}/news`} className="mt-4 text-blue-600 hover:underline">
                    Back to News
                </Link>
            </main>
        );
