...
import { AboutPageData, fallbackAboutData, TeamMember } from "@/lib/fallback-about-data";

// Helper to fetch data safely with fallback layer
async function fetchAboutData(lang: string): Promise<{ data: AboutPageData; isFallback: boolean }> {
    const token = process.env.API_TOKEN;
    try {
        const res = await dbFetch<{ data: AboutPageData }>(`client/about-us?lang=${lang}`, {
            headers: {
                'Cookie': `token=${token || ''}`
            },
        ...
        console.error(`[SSR] About fetch failed for '${lang}', using static fallback.`);
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const description = data.metadata?.description || "Apollo Global Interactive Legacy.";
