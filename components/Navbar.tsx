...
export default function Navbar({ lang = "en" }: { lang?: string }) {
	...
	const navLinks = [
		...
		{
			name: isId ? "Hubungan Investor" : "Investor Relations",
	...
	const isActiveLink = (link: (typeof navLinks)[0]) => {
		...
		const pathWithoutLang =
			pathRest.length > 0 ? "/" + pathRest.join("/") : "/";
		...
		if (pathWithoutLang === "/") {
			...
			// if (link.href === "/services" && activeSection === "services")
			// 	return true;
							...
							{navLinks.map((link) => {
								...
								return (
									<Link
										key={link.name}
										href={href}
										className={`relative py-1 text-[16px] font-medium transition-colors hover:text-[#5a80b9] ${
											isActiveLink(link)
										...
										return (
											...
											>
												<Link
													...
													className={`block w-full rounded-xl px-4 py-3 text-[16px] font-medium transition-colors ${
														isActiveLink(link)
